...
try {
  ...
  const title: string | undefined = document.querySelector('meta[property="og:title"]')?.getAttribute("content") || undefined;
  console.log(`Blog Title: ${title}`);

  console.log("\n--- Contextual Book Extraction ---");

  const booksWithContext: BookContext[] = [];
  const contentContainer: HTMLElement = (document.querySelector(".newsShowColumn") as HTMLElement) || (document.body as HTMLElement);
        ...
        if (href?.includes("/book/show/")) {
            ...
            if (match) {
                ...
                if (img) {
                    ...
                    if (imgAlt) candidate.title = imgAlt;
                ...
                if (lastBook && lastBook.id === id && lastBook.section === state.currentSection) {
                    if (!lastBook.title && candidate.title) lastBook.title = candidate.title;
                    if (!lastBook.coverImage && candidate.coverImage) lastBook.coverImage = candidate.coverImage;
                } else {
  ...
  const results: BookContext[] = booksWithContext.filter((b: BookContext) => b.title || b.coverImage);
  ...
  bySection.forEach((books: BookContext[], section: string) => {
      ...
      books.slice(0, 3).forEach((b: BookContext) => console.log(`   - ${b.title || "No Title"} (${b.id})`));
